...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private void OnTriggerEnter(Collider other)
        {
            ...
            if (this.TryGetComponent<SpeedBoost>(out var temp))
            {
                if (temp.isActivated)
                {
                    return;
        ...
        private bool _slowSpeedCheck = false;

        IEnumerator SlowSpeed()
        {
            if (_slowSpeedCheck == false)
            {
                _slowSpeedCheck = true;
            }
            
            SprintSpeed /= 2;
            yield return new WaitForSeconds(3f);
            SprintSpeed = _originalSpeed;
            _slowSpeedCheck = false;
        }
    }
}